...
static func run() -> void:
	...
	assert(not BreederNoteService.new(repository).add_note(lambs[1].sheep_id, "Promising fleece", 22).is_empty())
	...
	for original: SheepRecord in repository.all_sheep():
		...
		assert(restored.personality.temperament == original.personality.temperament and restored.personality.traits() == original.personality.traits())
		assert(restored.life_events.size() == original.life_events.size() and restored.breeder_notes.size() == original.breeder_notes.size())
	...
	assert(FlockPersistence.from_dictionary({"save_version":1, "sheep":[], "genetic_knowledge":[]}) == null and "Unsupported save version" in FlockPersistence.last_error)
