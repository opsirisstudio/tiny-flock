...
func get_statistics(sheep_id: String) -> Dictionary:
	...
	var fed := history.get_events_by_type(sheep_id, SheepLifeEvent.Type.FED)
	var favorite_food_count := 0
	for event: SheepLifeEvent in fed:
		if bool(event.metadata.get("favorite_match", false)): favorite_food_count += 1
	return {"offspring_count":breeding.get_offspring(sheep_id).size(), "breeding_count":breeding.get_breeding_count(sheep_id), "shearing_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.SHEARED).size(), "barn_archive_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.ARCHIVED_TO_BARN).size(), "pasture_return_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.RETURNED_FROM_BARN).size(), "favorite_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.MARKED_FAVORITE).size(), "known_descendant_count":descendants.size(), "known_generation_depth":max_depth, "feeding_count":fed.size(), "favorite_food_count":favorite_food_count, "petting_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.PETTED).size(), "grooming_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.GROOMED).size(), "washing_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.WASHED).size(), "treat_count":history.get_events_by_type(sheep_id, SheepLifeEvent.Type.TREAT_GIVEN).size()}
